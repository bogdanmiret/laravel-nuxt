            ...
            <v-flex xs12>
                ...
                >
                  ...
                  {{ item.numberFormated }}
                                ...
                                <b v-if="$auth.user.mainPhone && $auth.user.mainPhone.id === item.id">(для смс
                                    уведомлений)</b>
                            </td>
                            <td>{{ item.label }}</td>
                            ...
                            <td>
                                <v-checkbox @change="changePublicStatePhone(item)" :input-value="item.public"
                                            class="ml-4" hide-details/>
                            </td>
                            <td class="text-xs-center">
        ...
        <v-dialog v-model="deleteDialog" max-width="390">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn @click="deleteDialogDeletePhone" :loading="deleteDialogLoadingBtn" color="error" text>
                        Да
                    </v-btn>
                </v-card-actions>
            </v-card>
          ...
          <span class="headline">
            ...
            Для подставления префикса Вашей страны сохраните ее в настройках.
                    ...
                    <v-layout justify-start>
                        ...
                        <v-tooltip bottom>
                            <v-btn color="primary" class="mt-3" text icon>
                                <v-icon>info</v-icon>
                            </v-btn>
                    ...
                    <v-checkbox v-model="createDialogForm.main" label="Сделать главным(для смс уведомлений)?"
                                :disabled="!$auth.user.phones.length"/>
                </v-card-text>

                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" @click.native="createDialog = false" text>
                        Отмена
                    </v-btn>
                    <v-btn color="blue darken-1" @click.native="createDialogSavePhone" :loading="createDialogLoadingBtn"
                           :disabled="!!this.errors.items.length" text>
                        Сохранить
                    </v-btn>
                </v-card-actions>
            </v-card>
