<template>
    <v-scale-transition appear>
      <div v-if="userHaveGlobalMessage()">
        <v-alert
          v-if="underToolbar"
        ...
        >
          <v-slide-y-transition mode="out-in">
            <div v-if="underToolbarShowMiniMessage">
              ...
              <v-btn @click="underToolbarShowMiniMessage = false" color="primary" class="mt-1 ml-2" rounded small>
                Подробнее
              </v-btn>
            </div>
...
export default {
  ...
  computed: {
    ...
    getInfoByUserData () {
      return infoType => {
        const infoText = {
          noActivated: ['warning', 'Your account is not activated', 0],
          noEmailVerified: this.$auth.user.mainEmail ? ['info', `You have not verified mail ${this.$auth.user.mainEmail.email}.`, 1] : null,
          noEmail: ['info', 'There is no mail in your account', 2]
        }
        let index
