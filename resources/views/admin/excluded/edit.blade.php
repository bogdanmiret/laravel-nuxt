...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                @if(isset($word))
                    {!! Form::model($word, ['url' => route('admin.excluded-words.update', [ 'id' => $word['id']]), 'method' => 'put']) !!}
                @else
                    {!! Form::open(['url' => route('admin.excluded-words.store')], ['method' => 'post']) !!}
                @endif
                <div class="box-body">
                    <div class="box-body">
                                ...
                                @forelse($languages as $key => $locale)
                                    <div id="{{$locale}}" class="tab-pane fade @if($key == defaultLocale()) in active @endif">
                                        ...
                                        <div class="form-group {{ $errors->has('translation.' . $locale . '.name') ? ' has-error' : '' }}">
                                            ...
                                            {!! Form::text('translation['. $locale .'][word]', null, ['class' => 'form-control']) !!}
                                            {!! Form::checkError('translation.' . $locale . '.word', $errors)  !!}
                                        </div>
